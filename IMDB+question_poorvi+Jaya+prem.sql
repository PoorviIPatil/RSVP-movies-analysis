...
SELECT COUNT(ratings.median_rating) AS COUNT_MEDIAN_RATING_8
FROM ratings
INNER JOIN movie
...
-- Yes, German movies get more votes then Italian movies
...
SELECT 
SUM(CASE WHEN r.avg_rating>8 THEN 1 ELSE 0 END) AS 'Superhit movies',
SUM(CASE WHEN r.avg_rating BETWEEN 7 AND 8 THEN 1 ELSE 0 END) AS 'Hit movies',
SUM(CASE WHEN r.avg_rating BETWEEN 5 AND 7 THEN 1 ELSE 0 END) AS 'One-time-watch-movies',
SUM(CASE WHEN r.avg_rating<5 THEN 1 ELSE 0 END) AS 'Flop movies'
FROM ratings r
INNER JOIN genre g
...
SELECT n.name as actress_name, SUM(r.total_votes) AS total_votes, COUNT(rm.movie_id) as movie_id, Round(Sum(r.avg_rating)/Sum(r.total_votes),2) AS actress_avg_rating,
